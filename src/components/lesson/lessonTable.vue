  ...
  <v-data-table :headers="headers" :items="LessonList" height="calc(100vh - 12rem)" fixed-header
    ...
    <template v-slot:item.actions="{ item }">
      <div class="_flex _gap-3 ">
        ...
        <v-btn v-roles="'users|parent|student'" elevation="0" icon="fa-brands fa-apple-pay _text-sm" color="green"
               variant="tonal"
               size="small" @click="showTransactionForm(item)"></v-btn>


        <v-tooltip v-if="canRenewLesson(item)" activator="parent" location="top" text="Renew Lesson">
          <template v-slot:activator="{ props }">
            <v-btn v-bind="props" elevation="0" icon="fa-thin fa-calendar _text-sm" color="warning" variant="tonal"
                   size="small" @click="renewLesson(item)"></v-btn>
          </template>
        </v-tooltip>
